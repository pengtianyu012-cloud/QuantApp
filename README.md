...
- 已实现 SQLite v3 账户持久化与重启恢复、订单状态审计、每日净值快照、真实回撤风控、完整交易成本、部分成交续撮、基础策略引擎和日线回测骨架；10 个页面已接入服务层。
- 未实现功能会在界面中标注“尚未实现”。
...
应用启动时会在单个 SQLite 事务中自动将 v2 数据库升级到 v3，不删除既有订单和成交。升级前建议备份本地数据库；字段、状态映射和成本口径见 docs/MIGRATION_PHASE0.md。
